...
const getGoogleRedirectUri = () => process.env.GOOGLE_REDIRECT_URI || 'http://localhost:5000/api/google-fit/callback';
...
export const getAuthorizationUrlController = async (req, res) => {
  try {
    ...
    const authUrl = getAuthorizationUrl(clientId, redirectUri, GOOGLE_FIT_SCOPES);
...
export const handleOAuthCallback = async (req, res) => {
  try {
    ...
    if (googleFit) {
      ...
      dbOperations.push(GoogleFit.create({
        user: userId,
    ...
    dbOperations.push(
      User.findByIdAndUpdate(userId, { isGoogleFitConnected: true }, { new: false })
    );
    ...
    const savedConnection = await GoogleFit.findOne({ user: userId, isActive: true });
    const user = await User.findById(userId).select('isGoogleFitConnected');
    
    if (!savedConnection || !savedConnection.accessToken || !savedConnection.refreshToken) {
    ...
    const deepLinkUrl = `diabetesapp://google-fit-connected?success=false&error=${encodeURIComponent(errorMessage)}`;
...
export const connectGoogleFit = async (req, res) => {
  try {
    ...
    if (!clientId) {
      ...
      console.error('   Please check your backend/.env file and ensure GOOGLE_CLIENT_ID is set');
      return res.status(500).json({
        success: false,
        message: 'Google OAuth not configured. Please set GOOGLE_CLIENT_ID in backend/.env file.'
      });
    }
...
export const syncHealthData = async (req, res) => {
  try {
    ...
    if (new Date() >= googleFit.tokenExpiry) {
      ...
      googleFit.tokenExpiry = new Date(Date.now() + newTokenData.expiresIn * 1000);
    ...
    const [steps, calories, sleep, heartRateSeries, bloodGlucose, bloodPressure] = await Promise.allSettled([
    ...
    const heartRateAvg = heartRateSeries?.length
      ? Math.round(heartRateSeries.reduce((sum, hr) => sum + (hr.bpm || 0), 0) / heartRateSeries.length)
      : null;
    ...
    if (error?.response?.status === 401 || error?.response?.status === 403) {
      await GoogleFit.findOneAndUpdate({ user: req.user.id }, { isActive: false });
...
export const backgroundSyncGoogleFit = async () => {
  ...
  for (const gf of connections) {
    try {
      ...
      console.error('[Background Google Fit sync] error for user', gf.user, error?.message || error);
      if (error?.response?.status === 401 || error?.response?.status === 403) {
